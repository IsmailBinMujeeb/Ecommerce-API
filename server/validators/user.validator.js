...
export const PromoteUserToModeratorValidator = () => [
    ...
    body("permissions")
        .notEmpty().withMessage("permissions object is required")
        .bail()
    ...
    ...permissionsKeys.map(key =>
        body(`permissions.${key}`)
            .exists().withMessage(`permission '${key}' is required`)
            .bail()
            .isBoolean().withMessage(`permission '${key}' must be a boolean`)
            .toBoolean()
    ),
